        ...
        Object.keys(resources).forEach(key => {
            ...
            ['sell', 'buy'].forEach(type => {
                ...
                if (container) {
                    ...
                    btn.innerHTML = `<div class="resource-icon mx-auto mb-1">${res.icon}</div><div class="text-xs font-medium">${res.name}</div>`;
                    btn.setAttribute('aria-label', `${res.name} ${type === 'sell' ? 'verkaufen' : 'erhalten'}`);
                    btn.className = 'trade-button glass-panel text-center rounded-lg hover:bg-slate-600 focus:outline-none transition-transform active:scale-95';
                    btn.onclick = () => (type === 'sell' ? this.selectSell(key) : this.selectBuy(key));
